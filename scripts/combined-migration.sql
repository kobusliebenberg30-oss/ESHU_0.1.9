...
-- sha256 in the storage driver) â€” only the metadata row is now per-owner.
...
-- index â€” DROP INDEX silently does nothing. The constraint stayed live and
...
-- Track migrations as applied so Prisma migrate deploy is a no-op going forward
...
INSERT INTO "_prisma_migrations" (id, checksum, migration_name, finished_at, applied_steps_count) VALUES
  (gen_random_uuid()::text, 'manual', '20260509133544_init', NOW(), 1),
  (gen_random_uuid()::text, 'manual', '20260509145643_game_timing_split', NOW(), 1),
  (gen_random_uuid()::text, 'manual', '20260513212641_comments', NOW(), 1),
  (gen_random_uuid()::text, 'manual', '20260513221500_asset_owner_sha_unique', NOW(), 1),
  (gen_random_uuid()::text, 'manual', '20260513223000_asset_drop_global_sha_unique', NOW(), 1)
